...
class ElevenLabsTTSService(WordTTSService):
    ...
    async def _connect(self):
        try:
            ...
            self._keepalive_task = self.get_event_loop(
            ).create_task(self._keepalive_task_handler())
            ...
            async for message in self._websocket:
                ...
                if msg.get("audio"):
                    ...
                    frame = TTSAudioRawFrame(
                        audio, self._settings["sample_rate"], 1)
                    await self.push_frame(frame)

                if msg.get("alignment"):
                    word_times = calculate_word_times(
                        msg["alignment"], self._cumulative_time)
